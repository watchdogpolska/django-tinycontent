...
class TinyContentNode(template.Node):
    def __init__(
        self, args: list[FilterExpression], nodelist: template.NodeList
    ) -> None:
        self.args = args
        self.nodelist = nodelist

    def get_name(self, context: template.Context) -> str:
...
@register.tag
def tinycontent(parser: Parser, token: Token) -> TinyContentNode:
...
def tinycontent_simple(context: template.Context, *args: Any) -> str:
    if not args:
        raise TemplateSyntaxError("'tinycontent' tag takes arguments.")
